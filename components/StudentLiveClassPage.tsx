...
const StudentLiveClassPage: React.FC<StudentLiveClassPageProps> = ({ user, courses, onJoinClass }) => {
    ...
    return (
        <div className="mt-6 space-y-6">
            <div>
                <h2 className="text-3xl font-bold text-slate-900 dark:text-white mb-1">Live Classes</h2>
                <p className="text-slate-500 dark:text-slate-400">Join any ongoing classes for your enrolled courses.</p>
            </div>

            {activeLiveClasses.length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {activeLiveClasses.map(({ course, liveClass }) => (
                        <div key={liveClass.id} className="bg-white dark:bg-slate-800 p-6 rounded-xl border-2 border-red-500/50 flex flex-col items-center text-center shadow-lg shadow-red-500/10 animate-pulse-in">
                            <div className="text-red-500 mb-3 animate-pulse">
                                {ICONS.live}
                            </div>
                            <h3 className="text-xl font-bold text-slate-900 dark:text-white">{course.name}</h3>
                            <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">A live class is in session now!</p>
                            <button
                ...
                <div className="text-center py-16 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700">
                    <div className="text-slate-400 dark:text-slate-500 w-16 h-16 mx-auto">
                    ...
                    <h3 className="text-xl font-bold text-slate-900 dark:text-white mt-4">No Live Classes Right Now</h3>
                    <p className="text-slate-500 dark:text-slate-400 mt-1">Check back later or watch for a notification banner.</p>
                </div>
            )}
        </div>
    );

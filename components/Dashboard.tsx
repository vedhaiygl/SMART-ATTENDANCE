                ...
                <ul className="space-y-3">
                    {courses.slice(0, 3).map(course => (
                        course.sessions.slice(-2).map(session => (
                             <li key={session.id} className="flex justify-between items-center p-3 bg-slate-100 dark:bg-slate-700/50 rounded-lg">
                                <div className="flex items-center">
                                    <div className={`w-3 h-3 rounded-full mr-3 ${session.type === 'Online' ? 'bg-sky-400' : 'bg-green-400'}`}></div>
                                    <div>
                                        <p className="font-semibold text-slate-800 dark:text-slate-200">{course.name} - Session</p>
                                        <p className="text-xs text-slate-500 dark:text-slate-400">{new Date(session.date).toLocaleString()} ({session.type})</p>
                                    </div>
                                </div>
